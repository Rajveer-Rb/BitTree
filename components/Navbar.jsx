...
const Navbar = () => {
  ...
  return (
    <>
      {showNav && <nav className='bg-white  md:w-[90vw] mx-auto top-12 right-[4vw] rounded-full p-4 fixed flex justify-between'>

        {/* <img className='block md:hidden' src="/menu.svg" alt="" /> */}

        <button id="dropdownDefaultButton" onClick={() => setshowDropDown(!showDropDown)} onBlur={() => { setTimeout(() => { setshowDropDown(false) }, 300); }} data-dropdown-toggle="dropdownDelay" data-dropdown-delay="500" data-dropdown-trigger="hover" className="md:hidden text-white bg-white hover:bg-slate-400 font-medium rounded-lg text-sm px-3 py-2 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button"><img className='' src="/menu.svg" alt="" />
        </button>
